         ...
         <tbody>
            ...
            {section worker $WORKERS}
               ...
               <tr>
                 ...
                 <td align="right"><a href="{$smarty.server.PHP_SELF}?page={$smarty.request.page|escape}&action={$smarty.request.action|escape}&do=delete&id={$WORKERS[worker].id|escape}"><button type="button">Delete</button></a></td>
               </tr>
            {/section}
            {/nocache}
         </tbody>
...
<blockquote>
   <form action="{$smarty.server.PHP_SELF}" method="post">
      ...
      {$smarty.session.USERDATA.username}.<input type="text" name="username" value="user" size="10" maxlength="20" required> · <input type="text" name="password" value="password" size="10" maxlength="20" required>&nbsp;<input type="submit" value="Add New Worker">
   </form>
<blockquote>